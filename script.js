...
$(document).ready(function () {
 //variables
  ...
  //last city put from local storage into li
  lastCity = localStorage.getItem("storedCity")
...
//date displayed
  ...
  //submit button adds city to api call
  ...
  city = $(this).text();
    ...
    $.ajax({
      ...
      //current weather received and added to page
      console.log(response1);
      $('#des').text(response1.weather[0].description); //add description
      var temp = response1.main.temp; //get and add temp
      var mytemp = `Temperature: ${temp} F`;
      $('#temp').text(mytemp);
      var wind = response1.wind.speed; //get and add wind speed
      var mywind = `Wind Speed: ${wind}mph`;
      $('#wind').text(mywind);
      var hum = response1.main.humidity; //get and add humidity
      var myhum = `Humidity: ${hum}%`;
      $('#hum').text(myhum);
      $('#cityname').text(city); //get and add city name

      $('#date').text(today); //add today's date
      var myicon = response1.weather[0].icon; //add weather icon
      var iconsrc = `http://openweathermap.org/img/wn/${myicon}@2x.png`
      $('#icon').attr('src', iconsrc);

      //add city to list below
      myli = $('<li>').text(city);
      $(myli).addClass("list-group-item");
      $('ul').prepend(myli);
      $(myli).addClass("btn");
      localStorage.setItem('storedCity', city) //store city added below
    
     

    });

    //FORCAST STARTS - DAY 1
